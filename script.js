...
class PhotoScanner {
    ...
    setupEventListeners() {
        ...
        scanButton.addEventListener('click', () => this.scanImage());
    ...
    processImage(file) {
        ...
        img.onload = () => {
            ...
            financeManager.showNotification('Image loaded successfully! Click "Scan Text" to extract text.', 'success');
        };
    ...
    async scanImage() {
        ...
        try {
            ...
            // Display results
            document.getElementById('scannedText').value = result.data.text;
            
            financeManager.showNotification('Text scanned successfully!', 'success');

        } catch (error) {
            console.error('OCR Error:', error);
            financeManager.showNotification('Error scanning image. Please try again.', 'error');
        } finally {
            this.isScanning = false;
...
// Initialize photo scanner when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    financeManager = new FinanceManager();
    photoScanner = new PhotoScanner();
});

// Photo Scanner Utility Functions
function copyScannedText() {
    const scannedText = document.getElementById('scannedText');
    scannedText.select();
    document.execCommand('copy');
    financeManager.showNotification('Scanned text copied to clipboard!', 'success');
}

function clearScannedText() {
    ...
    document.getElementById('imageCanvas').classList.add('d-none');
...
function translateText() {
    ...
    if (!input) {
        financeManager.showNotification('Please enter text to translate', 'warning');
        return;
    }

    // Simple translation logic using our existing ukrainianVerses
    let translated = '';
    
    // Check if it's a Bible verse we have translated
    if (ukrainianVerses[input]) {
        translated = ukrainianVerses[input];
    } else {
        // For other text, provide a simple response
        translated = `[Translation not available for: "${input}"]\n\nNote: This tool primarily translates Bible verses. For other text, consider using the photo scanner to extract text from images.`;
    }
    
    result.value = translated;
    financeManager.showNotification('Translation completed!', 'success');
}

// Cleanup when page unloads
window.addEventListener('beforeunload', () => {
    if (photoScanner) {
        photoScanner.terminate();
    }
});
